...
RUN chmod +x /opt/startup.relational.dev.sh /opt/startup.relational.prod.sh
RUN sed -i 's/\r//g' /opt/wait-for-it.sh
RUN sed -i 's/\r//g' /opt/startup.relational.dev.sh
RUN sed -i 's/\r//g' /opt/startup.relational.prod.sh

WORKDIR /usr/src/app
# Do not bake env-example into the image — runtime config comes from compose/K8s env.
RUN npm run build
